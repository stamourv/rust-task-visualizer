...
#![feature(phase)]
#[phase(plugin, link)] extern crate green;
extern crate rtinstrument;

green_start!(real_main)

fn start(n_tasks: int, token: int) {
    let (donec, donep) = channel();
    let (tx, mut rx) = channel();
    tx.send(token);
    for i in range(2, n_tasks + 1) {
        let (tx, next_rx) = channel();
        let d2 = donec.clone();
        spawn(proc() {roundtrip(i, tx, rx);d2.send(())});
        rx = next_rx;
    }
    spawn(proc() {roundtrip(1, tx, rx);donec.send(())});

    for _ in range(1, n_tasks + 1) {
        donep.recv();
    }
}
